...
public class Board implements IKeyListener {
    ...
    private Shuttle_fondo shuttle_fondo;
    ...
    public Board(Dimension2D original) {
        ...
        this.shuttle = new Shuttle(new Point2D(this.game_zone.getMinX()+this.game_zone.getWidth()/2, this.game_zone.getMinY() + (this.game_zone.getMaxY()-this.game_zone.getMinY())));
        this.shuttle_fondo = new Shuttle_fondo(new Point2D(this.game_zone.getMinX()+this.game_zone.getWidth()/2, this.game_zone.getMinY() + (this.game_zone.getMaxY()-this.game_zone.getMinY())));
        this.original_size = original;
        this.bubblegrid = new Bubblegrid(this.game_zone);
    ...
    private void update() {
        //actualizar el juego
        iscolission = this.bubblegrid.collision(this.bubble);
        if(iscolission){
            this.bubble = null;
        }
        if (this.bubble != null && this.bubble != null) {
            this.bubble.move(this.game_zone);
        }

    }

    private void render() {
        this.shuttle_fondo.paint(this.gc);
        this.shuttle.paint(this.gc);
        if (bubble != null && bubble.getBalltype() != null) {
    ...
    public void paintBackground() {
        ...
        this.bggc.drawImage(fondos,16,545,this.original_size.getWidth(),this.original_size.getHeight(),
                0,0,
    ...
    public void onKeyPressed(KeyCode code) {
        switch (code) {
            case LEFT:
                ...
                this.shuttle.move_left();
                this.shuttle_fondo.move_left();
                break;
            case RIGHT:
                this.right_press = true;
                this.shuttle.move_right();
                this.shuttle_fondo.move_right();
                break;
        }
    }
    ...
    public void onKeyReleased(KeyCode code) {
        switch (code) {
            ...
            case ENTER:
                ...
                break;
            case SPACE:
                this.shuttle.shoot(this.game_zone);
                this.bubble = this.shuttle.getBubble();
                this.shuttle.setBubble(null);
                break;
            case P:
